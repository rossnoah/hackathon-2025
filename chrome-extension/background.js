// Background service worker for Clocked Moodle Sync
// Automatically syncs assignments every minute

console.log("🦇 Clocked background service worker started");
...
chrome.runtime.onInstalled.addListener(async () => {
	...
	console.log(
		`⏰ Alarm created: syncing every ${SYNC_INTERVAL_MINUTES} minute(s)`,
	);
});
...
async function performAutoSync() {
	...
	try {
		...
		const moodleUrl =
			"https://moodle.lafayette.edu/calendar/view.php?view=upcoming&course=1";

		// Check if there's already a Moodle tab open
		const tabs = await chrome.tabs.query({
			url: "https://moodle.lafayette.edu/calendar/*",
		});
